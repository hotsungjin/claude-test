...
use Bundle\Component\Referral\ReferralRewardService;
...
class OrderEndController extends \Controller\Front\Controller
{
    ...
    public function index()
    {
        try {
            ...
            // [설성목장 추천 리워드] 주문 완료 시 리워드 처리 (실패 주문 제외)
            if (!empty($orderInfo['orderNo']) && $orderInfo['orderStatus'] !== 'f') {
                try {
                    ReferralRewardService::getInstance()->processOrderReward(
                        $orderInfo['memNo'],
                        $orderInfo['orderNo'],
                        (int)$orderInfo['settlePrice']
                    );
                } catch (\Throwable $e) {
                    \App::getInstance('logger')->error('[Referral] OrderEnd 오류: ' . $e->getMessage());
                }
            }
