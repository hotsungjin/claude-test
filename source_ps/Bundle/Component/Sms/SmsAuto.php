...
class SmsAuto implements \SplSubject
{
     ...
     * @return bool
     */
    protected function availableKakaoAlrimLuna(): bool
    {
        return gd_is_plus_shop(PLUSSHOP_CODE_KAKAOALRIMLUNA) === true // 블룸에이아이(구: 루나소프트) 플러스샵 사용 여부
            && $this->kakaoAlrimIgnoreFlag == 'n' // 카카오 ignore 여부
            && $this->kakaoAlrimLunaAutoConfig['useFlag'] == 'y' // 블룸에이아이(구: 루나소프트) 사용 여부
            && $this->kakaoAlrimLunaAutoConfig[strtolower($this->smsType) . 'UseFlag'] == 'y'; // smsType 별 사용 여부
    }
     ...
     * @return bool
     */
    protected function availableKakaoAlrimCloud(): bool
    {
        return $this->kakaoAlrimIgnoreFlag == 'n' // 카카오 ignore 여부
            && $this->kakaoAlrimCloudAutoConfig['useFlag'] == 'y'  // 카카오 토클 사용 여부
            && $this->kakaoAlrimCloudAutoConfig[strtolower($this->smsType) . 'UseFlag'] == 'y' // smsType 별 사용 여부
            && $this->kakaoAlrimCloudConfig['useFlag'] == 'y';
    }
     ...
     * @return bool
     */
    protected function availableKakaoAlrimBizm(): bool
    {
        return $this->kakaoAlrimIgnoreFlag == 'n' // 카카오 ignore 여부
            && $this->kakaoAlrimAutoConfig['useFlag'] == 'y'  // 카카오 비즈엠 사용 여부
            && $this->kakaoAlrimAutoConfig[strtolower($this->smsType) . 'UseFlag'] == 'y';  // smsType 별 사용 여부
    }

    protected function checkKakaoAlrim(string $kakaoAlrimType = '')
    {
        $this->checkSmsTypeAndCodeWithReceiverKakao($kakaoAlrimType);
        $this->checkSmsAutoTypeAndSmsContentWheresKakao($kakaoAlrimType);
        $this->checkDefaultSmsAutoCodeAndReceiverKakao($kakaoAlrimType);
    ...
    public function autoSend($kakaoAlrimIgnoreFlag = 'n')
    {
        ...
        try {
            ...
            if ($this->availableKakaoAlrimLuna()) {
                // 블룸에이아이(구: 루나소프트)
                $this->checkKakaoAlrim(self::KAKAO_ALRIM_LUNA);
                ...
                $this->setKakaoAlrimAutoReserveTime(self::KAKAO_ALRIM_LUNA);
            } else if ($this->availableKakaoAlrimCloud()) {
                // 카카오 클라우드
                $this->checkKakaoAlrim(self::KAKAO_ALRIM_CLOUD);

                $smsContents = $this->getSmsContentsKakaoCloud($this->smsContentWheres);
                $logger->info(sprintf('Count sms contents.[%d]', count($smsContents)));

                $this->setKakaoAlrimAutoReserveTime(self::KAKAO_ALRIM_CLOUD);
            } else if ($this->availableKakaoAlrimBizm()) {
                // 카카오 비즈엠
                $this->checkKakaoAlrim();

                $smsContents = $this->getSmsContentsKakao($this->smsContentWheres);

                $logger->info(sprintf('Count sms contents.[%d]', count($smsContents)));
                // 카카오알림톡 자동 발송 예약시간 설정
                $this->setKakaoAlrimAutoReserveTime();
            ...
            foreach ($smsContents as $index => $item) {
                ...
                } elseif ($this->availableKakaoAlrimCloud()) {
                    if ($item['delFl'] !== 'n') {
                        $logger->info(sprintf('Unavailable kakao alrim cloud template, index[%d]', $index));
                        $isPossibleSend = false;
                    } else {
                        $contents = $this->replaceContentKakaoCloud($contents, $item['replaceCodes'] ?? []);
                        $item['contents'] = $contents;
                    }
                } elseif ($this->availableKakaoAlrimBizm()) {
                    if (strtoupper($item['inspectionStatus']) !== 'APR' || strtoupper($item['status']) === 'S') {
                        $logger->info(sprintf('Unavailable kakao alrim bizm contents template, index[%d]', $index));
                        $isPossibleSend = false;
                    } else {
                        $contents = $this->replaceContentsKakao($contents);
                ...
                if ($isPossibleSend) {
                    ...
                    if($this->availableKakaoAlrimLuna()) {
                        ...
                        $kakaoService->send($item, $receiverData, $this->replaceArguments, $this->smsAutoSendDate);
                        $result[] = [
                            'success' => 1,
                            'fail'    => 0,
                        ];
                        ...
                        // 특정 sms발송이면 블룸에이아이(구: 루나소프트)로 보내도록 처리
    ...
    public function replaceContentKakaoCloud(string $contents, array $usedReplaceCode): string
    {
        /** @var KakaoAlrimCloudSendService $kakaoServices */
        $kakaoServices = \App::getInstance(KakaoAlrimCloudSendService::class);

        // 게시판의 경우 mallNm과 작성자 명이 반대,,
        ...
        foreach ($usedReplaceCode as $replaceCode) {
            // #{} 제거
            preg_match('/#\{(.*?)\}/', $replaceCode, $matches);
            if (!isset($matches[1])) {
                continue;
            }
            // 입력받은 치환 코드 없으면 빈값 처리
            if (!array_key_exists($matches[1], $this->replaceArguments)) {
                $this->replaceArguments[$matches[1]] = '';
            }
        }
        return $kakaoServices->replaceCodeToContent($contents, $this->replaceArguments, $usedReplaceCode);
    }
    ...
    public function getSmsContentsKakaoCloud(array $wheres)
    {
        ...
        foreach ($wheres as $v) {
            ...
            if ($sendInfo['smsType'] !== $this->smsType) {
                $logger->warning(sprintf('Not found sms contents query is [%s]', $templateCode), [__METHOD__]);
                throw new \Exception(__('자동발송 타입이 다릅니다.'));
            }
    ...
    protected function checkSmsAutoTypeAndSmsContentWheresKakao($kind = '')
    {
        ...
        if ($this->smsType == \Component\Sms\SmsAutoCode::MEMBER && $this->smsAutoCodeType == \Component\Sms\Code::JOIN) {
            if ($config['smsDisapproval'] != 'y' && $this->replaceArguments['appFl'] == 'n') {
                throw new \Exception('Disapproval member');
            }
        }
    }
    ...
    protected function checkSmsTypeAndCodeWithReceiverKakao($kind = '')
    {
        ...
        } else if ($kind == self::KAKAO_ALRIM_CLOUD) {
