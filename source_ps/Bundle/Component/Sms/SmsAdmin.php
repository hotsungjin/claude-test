...
class SmsAdmin
{
    ...
    const ALPHA_SMS_POINT_LIST = 'https://alpha-www.nhn-commerce.com/userinterface/_godoConn/conf/sms.cfg';
    const REAL_SMS_POINT_LIST = 'https://www.nhn-commerce.com/userinterface/_godoConn/conf/sms.cfg';
     ...
     * @throws \Exception
     */
    private function notifyManagerTutorial()
    {
        ...
        $managerTutorialTopicService->produceTutorialStatusChanged(
            ManagerTutorialType::SMS_REGISTER,
            true
        );
    }
    ...
    public function getSmsPriceList()
    {
        ...
        $out = '';
        try {
            $url = \App::isDevelopment() ? self::ALPHA_SMS_POINT_LIST : self::REAL_SMS_POINT_LIST;
            $uri = parse_url($url);
            $fp = fopen($url, 'r');
            if ($fp) {
                $res = '';
