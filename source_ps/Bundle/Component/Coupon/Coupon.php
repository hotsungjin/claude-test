...
class Coupon
{
    ...
    {
        $this->validateCouponUsableBeforeSave($couponNo, $memNo, $memGroupNo);
        ...
        try {
            ...
            // 중복 쿠폰 발급 체크
            $this->validateDuplicateAfterSave($couponNo, $memNo);
     ...
     * @throws \Exception
     */
    public function validateCouponUsableBeforeSave(int $couponNo, int $memNo, int $memGroupNo)
    {
        if (!isset($memNo) || $memNo <= 0) {
            throw new \Exception('로그인 하셔야 합니다.');
        }
        ...
        $couponField = [
            'couponType', 'couponKind', 'couponSaveType',
            'couponDisplayType', 'couponDisplayStartDate', 'couponDisplayEndDate',
            'couponApplyMemberGroup', 'couponApplyMemberGroup',
            'couponAmountType', 'couponAmount', 'couponSaveCount'
        ];
        $couponData = $this->getCouponInfo($couponNo, implode(',', $couponField));
        if (!($couponData['couponType'] == 'y' && $couponData['couponKind'] == 'online' && $couponData['couponSaveType'] == 'down')) {
            throw new \Exception('삭제된 쿠폰입니다.');
        }

        if ($couponData['couponDisplayType'] == 'y'
            && !(strtotime($couponData['couponDisplayStartDate']) <= time() && strtotime($couponData['couponDisplayEndDate']) >= time())
        ) {
            throw new \Exception('발급 기간이 지난 쿠폰입니다.');
        }

        // 발급 회원등급 체크
        if ($couponData['couponApplyMemberGroup'] && $memGroupNo) {
            $applyMemberGroupArr = explode(INT_DIVISION, $couponData['couponApplyMemberGroup']);
            if (array_search($memGroupNo, $applyMemberGroupArr) === false) {
                throw new \Exception('발급 가능 회원등급이 아닙니다.');
            }
        }

        // 발급 수량 체크
        if ($couponData['couponAmountType'] == 'y' && $couponData['couponAmount'] <= $couponData['couponSaveCount']) {
            throw new \Exception('쿠폰 발급 개수가 초과되어 발급할 수 없습니다.');
        }
    }
    ...
    /**
     * 쿠폰 발급 후 → 회원 쿠폰 중복 발급 여부 확인
     *
     * @param int $memNo 회원고유번호
     * @param int $couponNo 쿠폰 고유번호
     * @return void
     * @throws \Exception
     */
    public function validateDuplicateAfterSave(int $couponNo, int $memNo)
    {
        $couponField = [
            'couponSaveDuplicateType', 'couponSaveDuplicateLimitType', 'couponSaveDuplicateLimit',
            'couponAmountType', 'couponAmount', 'couponSaveCount'
        ];
        $couponData = $this->getCouponInfo($couponNo, implode(',', $couponField));

        $saveMemberCouponCount = $this->getMemberCouponTotalCount($couponNo, $memNo);
        if ($couponData['couponSaveDuplicateType'] == 'n' && $saveMemberCouponCount > 1) {
            throw new \Exception('이미 발급 받은 쿠폰 입니다.');
        } else if ($couponData['couponSaveDuplicateType'] == 'y' && $couponData['couponSaveDuplicateLimitType'] == 'y' && $couponData['couponSaveDuplicateLimit'] < $saveMemberCouponCount) {
            throw new \Exception('쿠폰 발급 횟수가 초과되어 발급할 수 없습니다.');
        } else if ($couponData['couponAmountType'] == 'y' && $couponData['couponAmount'] < $couponData['couponSaveCount']) {
            throw new \Exception('쿠폰 발급 개수가 초과되어 발급할 수 없습니다.');
        }
    }
