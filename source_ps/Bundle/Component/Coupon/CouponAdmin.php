...
//        \Logger::debug($strSQL, $this->arrBind);
    ...
    public function deleteCoupon()
    {
        ...
        foreach ($getValue['chkCoupon'] as $key => $val) {
            ...
            $deletedCouponData = $this->getCouponInfo($couponNo, implode(',', $couponField));
            ...
            // 이미지 삭제
            if (isset($deletedCouponData['imageStorage']) && $deletedCouponData['imageStorage'] == 'obs') {
                ...
                try {
                    ...
                    if (ArrayUtils::isEmpty($files['couponImage']) === false) {
                        ...
                        if ($file['error'] == 0 && $file['size']) {
                            ...
                            $arrBind = $this->db->get_binding(DBTableField::tableCoupon(), $arrData, 'update', array_keys($arrData));
                            $this->db->bind_param_push($arrBind['bind'], 'i', $couponNo);
                    ...
                    if ($isImageUploaded && isset($arrData['imageStorage']) && isset($arrData['couponImage'])) {
            ...
            case 'modify' : {
                ...
                if (ArrayUtils::isEmpty($files['couponImage']) === false) {
                    ...
                    if ($file['error'] == 0 && $file['size']) {
                        ...
                        $couponField = ['imageStorage', 'couponImage'];
                        $couponDataBeforeUpdate = $this->getCouponInfo($couponNo, implode(',', $couponField));
                    }
                }
                ...
                if ($isImageUploaded && isset($couponDataBeforeUpdate['imageStorage']) && isset($couponDataBeforeUpdate['couponImage'])) {
    ...
    public function saveExcelMemberCoupon($sSmsFlag = 'n', $passwordCheckFl = true)
    {
        try {
            ...
            $excel = \App::load('\\Component\\Excel\\ExcelDataConvert');
            $result = $excel->setExcelMemberCouponUp($arrData, $sSmsFlag, $passwordCheckFl);
