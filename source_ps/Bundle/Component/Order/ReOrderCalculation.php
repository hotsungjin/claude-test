...
class ReOrderCalculation
{
    ...
    public function setRefundCompleteOrderGoodsNew($getData, $autoProcess, $userHandleFl = false)
    {
        ...
        if (empty($getData['refund']) === false) {
            $confirmRefundCompletion = $this->confirmRefundCompletion($getData['orderNo'], array_keys($getData['refund']));
        }

        foreach ($getData['refund'] as $handleSno => $aVal) {
            if (empty($confirmRefundCompletion) === false) {
                if (in_array($handleSno, $confirmRefundCompletion)) {
...
//            }
    ...
    */
    ...
    private function confirmRefundCompletion(string $orderNo, array $handleSno): array
    {
        ...
        $arrWhere[] = 'oh.sno IN (' . implode(' , ', $tmpAddWhere) . ')';
        unset($tmpAddWhere);
