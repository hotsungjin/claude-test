...
class OrderAdmin extends \Component\Order\Order
{
        ...
        if ($this->search['couponAllFl'] === 'y') {
            ...
            if (!empty($this->couponNumbers)) {
                ...
                $this->arrWhere[] = '(
                    ...
                    AND (mc.couponNo IN (' . implode(', ', $tmpAddWhere) . '))
                ) > 0';
    ...
    public function getOrderListForAdmin($searchData, $searchPeriod, $isUserHandle = false)
    {
        ...
        if ($isUserHandle) {
            ...
            $join[] = ' LEFT JOIN ' . DB_ORDER_USER_HANDLE . ' ouh ON (og.orderNo = ouh.orderNo) AND (og.userHandleSno = ouh.sno || (og.sno = ouh.userHandleGoodsNo && og.orderStatus IN (\'' . implode('\',\'', array_keys($joinOrderStatusArray)) . '\')))';
        }
        ...
        if ($page->hasRecodeCache('total') === false) {
            if (Manager::isProvider()) {
                // 검색된 주문의 개수
                $total = $this->db->query_fetch('SELECT COUNT(distinct(og.sno)) AS cnt FROM ' . $fromTable . implode(' ', str_ireplace('limit ' . $page->recode['start'] . ',' . $searchData['pageNum'], '', $query)), $this->arrBind, true);
                ...
                $fromTable = $isDisplayOrderGoods ? DB_ORDER_GOODS . ' og ' : DB_ORDER . ' o ';
                $totalPriceQuery = 'SELECT (o.realTaxSupplyPrice + o.realTaxFreePrice + o.realTaxVatPrice) AS price, COUNT(distinct(og.sno)) AS cnt FROM '. $fromTable . implode(' ', str_ireplace('limit ' . $page->recode['start'] . ',' . $searchData['pageNum'], '', $query));
        ...
        if ($this->search['exceptOrderStatus']) { // 예외처리할 주문상태 쿼리
            $exceptStatusQuery = sprintf("og.orderStatus NOT IN ('%s')", implode("','", $this->search['exceptOrderStatus']));
        }
...
//                        $fieldData['update']['userHandleSno'] = $bundleData['sno'];
            ...
            foreach ($goodsData as $key => $val) {
                ...
                if($channel != 'naverpay') {
                    if($userHandleFl == true){
                        ...
                        if(in_array(substr($val['orderStatus'],0,1), array('c','f','b','e')) || in_array($statusMode, array('d','s')) && $changeStatus == 'r1'){
                            $changeStatusCheck = false;
                        }
                    } else {
                        // 실제 주문상품의 주문상태
     ...
     */
    private function autoOrderCancelLog(string $orderNo, int $orderGoodsNo, bool $successFailure, string $failureMsg = '')
    {
    ...
    public function getOrderListForAdminExcel($searchData, $searchPeriod, $isUserHandle = false, $orderType='goods',$excelField,$page,$pageLimit)
    {
        ...
        if ($orderType === 'goods') {
            if (!preg_match("/orderInfoCd/", $orderSort)) {
                $orderSort = $orderSort . ", oi.orderInfoCd asc, og.orderCd asc";
            }
        }
