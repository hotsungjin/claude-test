...
class GoodsAdmin extends \Component\Goods\Goods
{
    ...
    public function getDataGoods($goodsNo = null, $taxConf, $applyFl = false)
    {
        ...
        $checkKeys = [
            ...
            'taxFreeFl', 'stockFl', 'soldOutFl', 'maxOrderChk', 'deliveryFl', 'deliveryFree', 'relationSameFl',
            'relationFl', 'qrCodeFl', 'goodsPermission', 'onlyAdultFl', 'goodsDiscountFl', 'salesDateFl',
            'addGoodsFl', 'imgDetailViewFl', 'externalVideoFl', 'goodsState', 'scmFl', 'externalVideoSizeFl',
            'mileageGroup', 'goodsDiscountGroup', 'exceptBenefitGroup', 'benefitUseType', 'goodsBenefitSetFl',
            'goodsAccess', 'goodsPermissionPriceStringFl', 'onlyAdultDisplayFl', 'onlyAdultImageFl',
    ...
    public function saveInfoGoods($arrData)
    {
        ...
        if(!Session::get('manager.isProvider')) { // 본사만 실행
            ...
            $themeAllData = $this->db->query_fetch($strSQL, $arrBind['bind']);
        ...
        */
        ...
        if($optionType['use'] == 'y' && $arrData['optionFl'] == 'y' && $arrData['optionReged'] != 'y'){
            ...
            if ($arrData['optionFl'] == 'y') {
                if ($arrData['option']['optionImageDeleteFl']) {
                    ...
                    if (in_array($arrData['imageStorage'], self::DEFAULT_STORAGE) && !empty($arrGoodsOptionIconWhere)) {
                        $strGoodsOptionIconWhere = implode('","', $arrGoodsOptionIconWhere);
        ...
        if($arrData['applyGoodsCopy'] && in_array($arrData['imageStorage'], self::DEFAULT_STORAGE)) {
        ...
        if (!empty($goodsImageMigrationLog)) {
            if (count($goodsImageMigrationLog['image']) < 1) {
                unset($goodsImageMigrationLog['image']);
            }

            if (count($goodsImageMigrationLog['imageIcon']) < 1) {
    ...
    protected function imageUploadIcon($arrFileData, $arrData, $arrOptionValue, $strImageStorage, &$goodsImageMigrationLog = [])
    {
        ...
        // 상품아이콘 > obs 카프카 컨슈머 처리 start
        if (in_array($strImageStorage, self::DEFAULT_STORAGE) && !empty($goodsImageLog)) {
    ...
    public function imageUploadGoods($arrFileData, $strImageStorage, $arrImageSize, $arrImageResize, $imageInfo,$goodsNo, $mode = null, &$goodsImageMigrationLog = [], array $extraArrData = [])
    {
            ...
            foreach ($_getImage as $k => $v) {
                if ($_getImageCompare[$v['sno']] == 'update' || $_getImageCompare[$v['sno']] == 'delete') {
                    if(in_array($v['imageName'],$tmpInfo['imageName']) === false || empty($tmpInfo) === true) {
                        ...
                        if (!empty($v['imageUrl'])) {
                            $tmpGoodsImageMigrationDeleteLog['imageUrl'] = $v['imageUrl'];
                        }
                        if (!empty($v['thumbImageUrl'])) {
                            $tmpGoodsImageMigrationDeleteLog['thumbImageUrl'] = $v['thumbImageUrl'];
                        }
                        if (!empty($tmpGoodsImageMigrationDeleteLog)) {
                            $goodsImageMigrationDeleteLog[] = $tmpGoodsImageMigrationDeleteLog;
                        }
                    }
                }
            }
        ...
        if (!empty($extraArrData['originalGoodsNo']) && in_array($strImageStorage, self::DEFAULT_STORAGE)) {
        ...
        if (in_array($strImageStorage, self::DEFAULT_STORAGE)) {
    ...
    public function setCopyGoods($goodsNo, $arrCopyData = [])
    {
        ...
        foreach ($arrGoodsTable as $goodsTableNm) {
            ...
            if ($functionNm == 'TableGoods') {
                ...
                if (!GoodsImageHelper::isStorageTypeSupportImageCopy($data['imageStorage'])) {
                    $addExcludeField = ['iconImage', 'goodsImage', 'iconImageUrl', 'iconThumbImageUrl'];
                    $excludeField = array_merge($excludeField, $addExcludeField);
                }
            ...
            // 상품이미지 > obs 카프카 컨슈머 처리 start
            if(in_array($data['imageStorage'], self::DEFAULT_STORAGE)) {
                ...
                if ($functionNm == 'TableGoodsImage') {
                    $strSQL = 'INSERT INTO ' . $goodsTableNm . ' (regDt,goodsNo,goodsImageStorage,imageUrl,thumbImageUrl,obsConvertFl,' . implode(', ', $fieldData) . $addField . ') SELECT now(),\'' . $newGoodsNo . '\', "' . $goodsImageStorage . '", "'.$tempImageUrl.'", "'.$tempImageUrl.'", "'.$obsConvertFl.'", ' . implode(', ', $insertFieldData) . $addData . ' FROM ' . $goodsTableNm . ' WHERE goodsNo = ' . $goodsNo;
                } else if ($functionNm == 'TableGoodsOptionIcon') {
                    $strSQL = 'INSERT INTO ' . $goodsTableNm . ' (regDt,goodsNo,optionImageStorage,goodsImageUrl,goodsThumbImageUrl,obsConvertFl,' . implode(', ', $fieldData) . $addField . ') SELECT now(),\'' . $newGoodsNo . '\', "' . $goodsImageStorage . '", "'.$tempImageUrl.'", "'.$tempImageUrl.'", "'.$obsConvertFl.'", ' . implode(', ', $insertFieldData) . $addData . ' FROM ' . $goodsTableNm . ' WHERE goodsNo = ' . $goodsNo;
                }
            }
            // 상품이미지 > obs 카프카 컨슈머 처리 end
        ...
        if (!empty($arrUpdateSetSQL)) {
            $strUpdateSQL = "UPDATE " . DB_GOODS_SEARCH . " SET " . implode(' , ', $arrUpdateSetSQL) . " WHERE goodsNo = '" . $newGoodsNo . "' ";
            $this->db->query($strUpdateSQL);
        }
    ...
    function deleteGoodsObsImage($goodsNo)
    {
        try {
            ...
            $query = $this->db->query_complete();
            $strSQL = 'SELECT ' . array_shift($query) . ' FROM ' . DB_GOODS_IMAGE . implode(' ', $query);
    ...
    function deleteGoodsOptionObsImage($goodsNo)
    {
        try {
            ...
            $this->db->bind_param_push($arrBind, 'i', $goodsNo);
            $query = $this->db->query_complete();
            $strSQL = 'SELECT ' . array_shift($query) . ' FROM ' . DB_GOODS_OPTION_ICON . implode(' ', $query);
            $images = $this->db->query_fetch($strSQL, $arrBind);
            $obs = new ImageUploadService();
            foreach ($images as $image) {
    ...
    public function setDelStateGoods($goodsNo, $modDtUse = null)
    {
        ...
        // 상품 삭제 웹훅 추가
        $goodsWebHookService = \App::getInstance(GoodsWebHookService::class);
        $goodsWebHookService->sendGoodsWebHook(implode(",", $goodsNo), 'DELETED');
    ...
    public function setSoldOutGoods($goodsNo, $modDtUse = null)
    {
        ...
        // 상품 수정 웹훅 추가
        $goodsWebHookService = \App::getInstance(GoodsWebHookService::class);
        $goodsWebHookService->sendGoodsWebHook(implode(',', $goodsNo), 'UPDATED');
    ...
    public function setSearchGoods($getValue = null, $list_type = null)
    {
        ...
        $checkKeys = [
            'daumFl', 'naverFl', 'paycoFl', 'fbeFl', 'googleFl',
            'purchaseNoneFl', 'stockStateFl', 'addGoodsFl', 'applyType', 'applyFl',
            'goodsDeliveryFl', 'categoryNoneFl', 'brandNoneFl', 'scmFl', 'optionFl',
            'mileageFl', 'optionTextFl', 'goodsDisplayFl', 'goodsSellFl',
            'goodsDisplayMobileFl', 'goodsSellMobileFl', 'stockFl', 'soldOut',
            'goodsIconCdPeriod', 'goodsIconCd', 'deliveryFl', 'deliveryFree',
            'mobileDescriptionFl', 'restockFl', 'goodsBenefitNoneFl', 'sellStopFl',
            'confirmRequestFl', 'optionSellFl', 'optionDeliveryFl'
        ];
    ...
    protected function setAdminListGoods(&$getData,$addField = null,$mode = null)
    {
        ...
        $join = ' LEFT JOIN (
            ...
            FROM (
                  ...
                  FROM es_goodsImage
                  WHERE goodsNo IN ("'.implode('","',array_column($getData, 'goodsNo')).'")
        ...
        /* 이미지 설정 */
        $strImageSQL = 'SELECT goodsNo, imageName, imageUrl, thumbImageUrl, goodsImageStorage FROM ' . DB_GOODS_IMAGE . ' g  WHERE imageKind = "List" AND goodsNo IN ('.implode(',',array_column($getData, 'goodsNo')).')';
        $tmpImageData = $this->db->query_fetch($strImageSQL);
        $imageData = [];
        foreach (array_column($tmpImageData, 'goodsNo') as $goodsNo) {
            $index = array_search($goodsNo, array_column($tmpImageData, 'goodsNo'));
    ...
    public function getAdminListGoods($mode = null, $pageNum = 5)
    {
        ...
        if ($this->search['goodsIconCdPeriod'] && $this->search['goodsIconCd']) {
            ...
            $formatSQL = 'SELECT %s FROM ((SELECT ' . array_shift($query) . ' FROM ' . $this->goodsTable . ' g ' . implode(' ', $query) . ') UNION';
            ...
            if ($this->search['goodsIconCd']) {
                ...
                $selectSQL = ' (SELECT ' . array_shift($query2) . ' FROM ' . $this->goodsTable . ' g ' . implode(' ', $query2) . ')) a ';
    ...
    public function setBatchPrice($getData)
    {
        ...
        } else //선택된값 필드값 적용
        {
            ...
            // 상품 수정 웹훅 추가
            $goodsWebHookService->sendGoodsWebHook(implode(',', $getData['arrGoodsNo']), 'UPDATED');
        }
        ...
        if (is_array($arrData['goodsNoData'])) {
            if (gd_isset($arrData['tabGoodsCnt']) && is_array($arrData['tabGoodsCnt'])) {
                ...
                foreach ($arrData['tabGoodsCnt'] as $tabGoodsCnt) {
                    ...
                    $goodsNoData[] = array_slice($arrData['goodsNoData'], $startGoodsNo, $tabGoodsCnt);
    ...
    public function saveInfoDisplayThemeImage($arrFileData)
    {
        ...
        if ($arrFileData) {
            if (gd_file_uploadable($arrFileData, 'image')) {
                ...
                $newImageName =  $giftNo.'_'.rand(1,100) .  $imageExt; // 이미지 공백 제거 및 각 복사에 따른 종류를 파일명에 넣기
    ...
    public function getTmpGoodsImage($req = null, $isPaging = true , $isGroupByImageName = true)
    {
        ...
        if($isGroupByImageName){
            ...
            if (!empty($searchWhere)) {
                $this->db->strWhere = implode(' AND ', $searchWhere);
            }
            ...
            $query = $this->db->query_complete(false);
            $strSQL = 'SELECT ' . array_shift($query) . ' FROM ' . DB_TMP_GOODS_IMAGE . ' AS a ' . implode(' ', $query);
            $data = $this->db->query_fetch($strSQL, $arrBind);
            ...
            $query = $this->db->query_complete();
            $searchCountSQL = 'SELECT ' . array_shift($query) . ' FROM ' . DB_TMP_GOODS_IMAGE . ' AS a ' . implode(' ', $query);
    ...
    */
     ...
     * @return string
     */
    private function getSessionString(array $data)
    {
        //임시 세션값이 없다면 생성
        if (empty($data['sess'])) {
            do {
