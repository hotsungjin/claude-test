...
/**
 * CipherConvertor CLI 실행
 *
 * @run
 *      /usr/local/php/bin/php /www/[USER_HOME]/route.php job --name="CliCipherConvert"
 *
 * @package Bundle\Component\Cli
 */
class CliCipherConvert extends AbstractJob
{
    /**
     * Job 실행 전 처리 로직
     *
    ...
    protected function execute()
    {
        if (php_sapi_name() == 'cli') {
            ...
            $this->logMessage('[Convert] Start: ' . $startTime);

            $cipherConvertorService = ConvertorServiceFactory::createConvertorService();
            $cipherConvertorService->convert();

            $endTime = date('Y-m-d H:i:s');
            $this->logMessage('[Convert] End: ' . $endTime . ' (Start: ' . $startTime . ')');
        }
