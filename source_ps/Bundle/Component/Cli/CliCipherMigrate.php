...
/**
 * CipherConvertor Migrate CLI 실행
 *
 * @run
 *      /usr/local/php/bin/php /www/[USER_HOME]/route.php job --name="CliCipherMigrate"
 *
 * @package Bundle\Component\Cli
 */
class CliCipherMigrate extends AbstractJob
{
    /**
     * Job 실행 전 처리 로직
     *
    ...
    protected function execute()
    {
        if (php_sapi_name() == 'cli') {
            ...
            $this->logMessage('[Migrate] Start: ' . $startTime);

            $cipherConvertorService = ConvertorServiceFactory::createConvertorService();
            $cipherConvertorService->migrate();

            $endTime = date('Y-m-d H:i:s');
            $this->logMessage('[Migrate] End: ' . $endTime . ' (Start: ' . $startTime . ')');
        }
