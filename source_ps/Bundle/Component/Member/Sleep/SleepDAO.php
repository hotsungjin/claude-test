...
class SleepDAO extends \Component\AbstractComponent
{
    ...
    {
        if (is_array($data)) {
            $arrBind['where'] = 'sleep.' . $field . ' IN(' . implode(',', array_fill(0, count($data), '?')) . ')';
            foreach ($data as $no) {
        ...
        $strSQL = 'SELECT ' . array_shift($query) . ' FROM ' . DB_MEMBER_SLEEP . ' AS sleep ' . implode(' ', $query);
    ...
    public function selectListBySearch(array $params)
    {
        ...
        $this->db->strField =  implode(',', DBTableField::setTableField('tableMemberSleep', null, ['encryptData'], 'ms')) . ', m.mallSno, sum(case when mc.memNo is null then 0 else 1 end) as cnt, mn.snsTypeFl';
