...
export async function POST(req: NextRequest) {
  try {
    ...
    if (body.purpose === 'signup') {
      const { data: existing } = await (supabase as any).from('members').select('id, auth_id').eq('phone', phone).single()
      if (existing) {
        // auth_id 없는 마이그레이션 회원이면 activate로 안내
        if (!existing.auth_id) {
          return NextResponse.json({ error: '기존 설성목장 회원입니다. 비밀번호 설정이 필요합니다.', needActivate: true }, { status: 409 })
        }
        return NextResponse.json({ error: '이미 가입된 휴대폰 번호입니다.' }, { status: 409 })
      }
    }

    // 계정 활성화 시 마이그레이션 회원인지 확인
    if (body.purpose === 'activate') {
      const { data: existing } = await (supabase as any).from('members').select('id, auth_id').eq('phone', phone).single()
      if (!existing) {
