...
## Supabase 설정
- **프로젝트 ID**: jjlcrcscyzmfcgjshnvo
- **초기 관리자**: hotsungjin@gmail.com / gmail에 자동 등록됨. 
...
## 필수 환경변수 (.env)
```
SUPABASE_URL=https://rvsoelpfeqlhuycjgusy.supabase.co
...
- **@PROGRESS.md**: 개발 진행 상황, 완료/미완료 작업 (작업 후 반드시 업데이트)
...
- 필요시 CLAUDE.md 와 ARCHITECTURE.md, URL_STRUCTURE.md 파일로 개선해주세요
- 모든 작업은 다음 주요 파일을 확인하여 작업해주세요
  - **@CLAUDE.md**: 전체 프로젝트 개요 및 기술스택과 작업 가이드
  - **@ARCHITECTURE.md**: 시스템 구조, DB 스키마, API 명세
  - **@PROGRESS.md**: 개발 진행 상황, 완료/미완료 작업 (작업 후 반드시 업데이트)
  - **@DESIGN.md**: UI/UX 디자인 가이드
    - wireframes 하위폴더에 UI 구현이 필요한 모든 화면은 xml 포맷으로 UI 화면 표현
  - **@TESTPLAN.md**: 테스트 항목
